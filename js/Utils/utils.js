...
export function addMultipleCoordinates(multiLine, coordinates, originPoint = {x: 0, y: 0}) {
    coordinates.forEach(coordinate => {
        const x = coordinate.x * 100 + originPoint.x * 100; // Convert to percentage
        const y = coordinate.y * 100 + originPoint.y * 100;
...
export function cartesianToPolar(centerPos, pos, radius) {
    // Calculate the Cartesian coordinates relative to the center of the sphere
    const x = pos.x - centerPos.x;
    const y = pos.y - centerPos.y;
    const z = pos.z - centerPos.z;
...
let testPos = {x: 1, y: 1, z: 0};
let radius = dist3d(centerPos, testPos);
let polar = cartesianToPolar(centerPos, testPos, radius);
...
export function rotateVector2d(vector, angle) {
    ...
    const rotatedY = y * cosA - x * sinA;

    return { x: rotatedX, y: rotatedY };
}

export function getRandomSpherePos(maxRadius) {
    const radius = randomInt(-maxRadius, maxRadius);
...
export function getVectorRectangleIntersection(vx, vy, width, height) {
    ...
    const pRight = { x: halfW, y: k * halfW };
    const pLeft = { x: -halfW, y: -k * halfW };

    const pTop = { x: halfH / k, y: halfH };
    const pBottom = { x: -halfH / k, y: -halfH };
    ...
    if (vx > 0 && vy > 0) {
        ...
        // bottom left
        points = [pLeft, pBottom];
    } else if (vx > 0 && vy < 0) {
        // bottom right
        points = [pRight, pBottom];
    }
    // check which of the intersection points are inside the rectangle
    points = points.filter( p => p.x >= -halfW && p.x <= halfW && p.y >= -halfH && p.y <= halfH );
...
export function quaternionFromAxisAngle(dir, angle) {
    ...
    const q = {x: v.x * sinA, y: v.y * sinA, z: v.z * sinA, w: Math.cos(halfAngle)};
    return q;
}
